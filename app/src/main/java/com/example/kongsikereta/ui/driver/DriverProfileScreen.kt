...
fun DriverProfileScreen(
    driverProfileScreenViewModel: DriverProfileScreenViewModel,
    onLogOutClicked: () -> Unit
) {
    val context = LocalContext.current

    if(!driverProfileScreenViewModel.isDataFetched || PreferencesManager.getBooleanPreference("isPreviousUser")){
        Log.i("isLaunchedEffectINvoked","yes")
        LaunchedEffect(driverProfileScreenViewModel.isDataFetched) {
            driverProfileScreenViewModel.loadDriverProfile(context)
            driverProfileScreenViewModel.isDataFetched = true
        }
    }
    val scrollState = rememberScrollState()
    val profileUiState = driverProfileScreenViewModel.driverProfileUiState.collectAsState()
    ...
    Column(
        ...
        ElevatedCard {
            Column(
                ...
                Text(text = "Vehicle Model  :  ${profileUiState.value.driver.vehicle.model}")
                Spacer(modifier = Modifier.height(16.dp))
                Text(text = "Capacity  :  ${profileUiState.value.driver.vehicle.capacity}")
                Spacer(modifier = Modifier.height(16.dp))
                Text(text = "Extra Features  :  ${profileUiState.value.driver.vehicle.notes}")
            }
        }
        Spacer(modifier = Modifier.height(64.dp))
        Button(onClick = { driverProfileScreenViewModel.logOut()
